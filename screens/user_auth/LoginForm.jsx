...
export default function LoginForm(props){
    ...
    const loginHandler = async () => {
        const response = await fetch(`https://sean-aaccd-default-rtdb.asia-southeast1.firebasedatabase.app/users.json`);
    ...
    return <View className={'flex flex-1 p-5 justify-center items-center bg-[#71bff0]'}>
        ...
        <View className={'flex flex-row mt-2'}>
            <TouchableOpacity onPress={loginHandler} className={'rounded-lg py-3 w-full bg-blue-700'}>
                <Text className={'text-center text-[#ffffff]'}>Login</Text>
            </TouchableOpacity>
        </View>
        <View className={'flex flex-row mt-5'}>
            <Text>Not yet a member? </Text>
            <TouchableOpacity onPress={()=> setIsLogin(false)}><Text className={'text-white'}>Signup</Text></TouchableOpacity>
        </View>
    </View>
}
